    ...
    pub fn fit(&mut self, features: &Vec<&Vec<f64>>, t: &Vec<f64>, num_iter: i32) {
        ...
        for _epoch in 0..num_iter {
            for j in 0..t.len() {
                ...
                let updated_bias = self.bias + eta * residue;

                /* updated w calculation */
                let mut updated_w = vec![];
                for i in 0..self.ndim {
                    let updated_w_i = self.w[i] + eta * residue * x[i];
                ...
                for f in 0..self.k {
                    ...
                    for i in 0..self.ndim {
                        ...
                        let updated_v_i_f: f64 = self.v[i][f] + eta * residue * nabra;
